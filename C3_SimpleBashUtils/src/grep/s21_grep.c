...
void parser(int argc, char * argv[], options *opt, char pattern[]) {
  ...
  if (argv[optind]) {
    if (opt->e && !opt->f) pattern[strlen(pattern)-1] = '\0';
    if (strlen(pattern) == 0) s21_strcat(pattern, argv[optind]);
...
void printLine(FILE * file, char pattern[], options opt, char *nameFile) {
  ...
  while (ext) {
    ...
    if (line == NULL) {
      ...
      opt.count++;
      if (!opt.c && !opt.l) {
        if (nameFile && !opt.h) printf("%s:", nameFile);
        if (opt.n) printf("%d:", lineCount);
        if (opt.o) printf("%s\n", pattern);
        if (!opt.o) printf("%s", tmp_line);
  ...
  if (opt.c && nameFile) printf("%s:", nameFile);
  if (opt.c) printf("%d\n", opt.count);  // c
}
